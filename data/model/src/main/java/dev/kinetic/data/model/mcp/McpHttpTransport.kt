...
internal fun publicAddress(address: InetAddress): Boolean {
    ...
    if (bytes.size == 16) return bytes[0] in 0x20..0x3f &&
        !(bytes[0] == 0x20 && bytes[1] == 1 && (bytes[2] < 2 || (bytes[2] == 0x0d && bytes[3] == 0xb8))) &&
        !(bytes[0] == 0x20 && bytes[1] == 2) && !(bytes[0] == 0x3f && bytes[1] == 0xff)
...
class McpHttpTransport internal constructor(base: OkHttpClient) : McpTransport {
            ...
            call.enqueue(object : Callback {
                override fun onFailure(call: Call, e: IOException) {
                    if (continuation.isActive) continuation.resumeWithException(McpException(when (e) {
                        is InterruptedIOException -> McpError.MCP_TIMEOUT
                ...
                override fun onResponse(call: Call, response: Response) {
                    try {
                        ...
                        if (continuation.isActive) continuation.resumeWithException(when (failure) {
                            ...
                            is InterruptedIOException -> McpException(McpError.MCP_TIMEOUT)
    ...
    private fun read(response: Response, id: String): JsonObject {
        ...
        if (type.parameter("charset")?.equals("utf-8", ignoreCase = true) == false) reject()
        ...
        if (type.type == "application" && type.subtype == "json") {
            ...
            val result = envelope(strictJson(decode(bytes.toByteArray())).jsonObject, id)
            if (!response.isSuccessful) reject(McpError.MCP_REMOTE_ERROR)
            return result
        }
    ...
    private fun envelope(value: JsonObject, id: String): JsonObject {
        ...
        value["error"]?.let { element ->
            val error = element as? JsonObject ?: reject()
            val code = error["code"] as? JsonPrimitive ?: reject()
            if (code.isString || code.longOrNull == null) reject()
            error.text("message") // Validate, but never retain or display remote diagnostic text.
            reject(if (code == JsonPrimitive(-32022)) McpError.MCP_UNSUPPORTED_PROTOCOL else McpError.MCP_REMOTE_ERROR)
        }
